...
from utils.dataloaders import fashionmnist_dataloaders
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', type=str, default="./fashionmnist", help="Path to dataset")
    ...
    path = os.path.abspath(os.path.abspath("./fashionmnist"))
    ...
    if args.download:
        ...
        # download
        dataset = torchvision.datasets.FashionMNIST(root=path, train=True, download=True, transform=transform_train)

        url = 'https://drive.google.com/uc?id=1GY6C3GmEj01OEIYFhJ7lM-zmf4qNEE5b'
        output = os.path.join(path, 'fashionmnist_c20_6test.tar.gz')
        print("\nDownload ...")
        gdown.download(url, output, quiet=False)
        # check
        md5 = '9076e7f7478c3eeb73a8496e0e2e86f0'
        ...
        emds = []
        for i in range(0, 6):
            cdataloders = fashionmnist_dataloaders(root="./fashionmnist",
                                            index_path="./fashionmnist/test{}/index.json".format(i),
                                            show=False)
    ...
    train_datasets = torchvision.datasets.FashionMNIST(root=path, train=True, download=True, transform=transform_train)
