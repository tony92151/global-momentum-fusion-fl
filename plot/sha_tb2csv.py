...
if __name__ == '__main__':
    ...
    for cr in [0.1, 0.3, 0.5, 0.7, 0.9]:
        path = os.path.join(args.tb_event, "sha_1l_DGC_cr{}".format(cr))
        _, r = tflog2pandas(path)
        result["DGC({})".format(cr)] = r["value"]

        path = os.path.join(args.tb_event, "sha_1l_GMC_cr{}".format(cr))
        _, r = tflog2pandas(path)
        result["GMC({})".format(cr)] = r["value"]

        path = os.path.join(args.tb_event, "sha_1l_GFDGC_cr{}".format(cr))
        _, r = tflog2pandas(path)
        result["DGCwGMF({})".format(cr)] = r["value"]

        path = os.path.join(args.tb_event, "sha_1l_DGCwGM_cr{}".format(cr))
        _, r = tflog2pandas(path)
        result["DGCwGM({})".format(cr)] = r["value"]

    pd.DataFrame(result).to_csv(os.path.join(args.output_csv, "traffic.csv"))