...
def save_both(data, CR, type=0, output="./plot", smooth_rate=0.5):
    # datas = data["train"]
    plt.rcParams["font.family"] = "Times New Roman"
    plt.figure(figsize=(12, 8))
    epoch = list(range(len(data["test"]["DGC"])))
    plt.plot(epoch,
    ...
    if type == 0:
        plt.xlim(0, len(data["test"]["DGC"]))
        plt.ylim(0, 0.5)
        plt.title('Top-1 Accuracy of Shakespeare (compression rate {})'.format(CR), fontsize=20, pad=10)
        ...
        plt.xlim(0, len(data["test"]["DGC"]))
        plt.ylim(1.5, 5.0)
        plt.title('Loss of Shakespeare (compression rate {})'.format(CR), fontsize=20, pad=10)
        plt.xlabel('Epoch', fontsize=20)
    ...
    save_pdf_path = os.path.join(output, "figure", save_type, "mix_plots_{}_{}.pdf".format(CR, save_type))
    os.makedirs(os.path.dirname(save_pdf_path), exist_ok=True)
    plt.savefig(save_pdf_path, transparent=True)


def save_baseline(data, output="./plot", smooth_rate=0.5):
    plt.rcParams["font.family"] = "Times New Roman"
    ...
    ax1.plot(x, smooth(data["test"]["DGC"], smooth_rate),
             '-', linewidth=4, markersize=6, marker="o", color="#2079B6", label='DGC')
    ...
    plt.title('Shakespeare Dataset', fontsize=20, pad=10)
    ...
    save_pdf_path = os.path.join(output, "figure", "baseline", "baseline_plots.pdf")
    os.makedirs(os.path.dirname(save_pdf_path), exist_ok=True)
    plt.savefig(save_pdf_path, transparent=True)
...
if __name__ == '__main__':
    ...
    for cr in tqdm([0.1, 0.3, 0.5, 0.7, 0.9]):
        ...
        train_csv = pd.read_csv(os.path.join(args.csv, "train_acc.csv"), index_col=0)
        test_csv = pd.read_csv(os.path.join(args.csv, "test_acc.csv"), index_col=0)
        ...
        save_both(data=data, CR=cr, type=0, output=args.output, smooth_rate=smooth_rate)

        # loss
        data = {"train": {}, "test": {}}

        train_csv = pd.read_csv(os.path.join(args.csv, "train_loss.csv"), index_col=0)
        test_csv = pd.read_csv(os.path.join(args.csv, "test_loss.csv"), index_col=0)
        ...
        save_both(data=data, CR=cr, type=1, output=args.output, smooth_rate=smooth_rate)
    ...
    test_csv = pd.read_csv(os.path.join(args.csv, "test_acc.csv"), index_col=0)
    traffic_csv = pd.read_csv(os.path.join(args.csv, "traffic.csv"), index_col=0)
    ...
    save_baseline(data=data, output=args.output, smooth_rate=smooth_rate)
